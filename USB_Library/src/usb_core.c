...
{
    ENDPOINT_INFO *pEP_info = &pInformation->Ctrl_Info;
    u16            save_rLength;
    u8            *Buffer;
    u16            Length;

    save_rLength = pEP_info->Usb_rLength;

    if (pEP_info->CopyData && save_rLength) {
        Length = pEP_info->PacketSize;
        if (Length > save_rLength)
            Length = save_rLength;

        Buffer = (*pEP_info->CopyData)(Length);
        pEP_info->Usb_rLength -= Length;
        pEP_info->Usb_rOffset += Length;

        PMAToUserBufferCopy(Buffer, GetEPRxAddr(ENDP0), Length);
    }

    if (pEP_info->Usb_rLength != 0) {
    ...
    /* Set the next State */
    if (pEP_info->Usb_rLength >= pEP_info->PacketSize) {
        pInformation->ControlState = OUT_DATA;
    } else {
        if (pEP_info->Usb_rLength > 0) {
            pInformation->ControlState = LAST_OUT_DATA;
        }
        else if (pEP_info->Usb_rLength == 0) {
...
{
    ENDPOINT_INFO *pEP_info = &pInformation->Ctrl_Info;
    u16            save_wLength = pEP_info->Usb_wLength;
    u8             ControlState = pInformation->ControlState;
    u8            *DataBuffer;
    u16            Length;
    ...
    DataBuffer = (*pEP_info->CopyData)(Length);

    UserToPMABufferCopy(DataBuffer, GetEPTxAddr(ENDP0), Length);

    SetEPTxCount(ENDP0, Length);

    pEP_info->Usb_wLength -= Length;
    pEP_info->Usb_wOffset += Length;
