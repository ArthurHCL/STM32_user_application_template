...
static const unsigned short ee_addr_table[EE_NUM_ADDR] = {
    EE_DFU_BOOT
};
...
/**
  ...
  */
static uint16_t EE_FindValidPage(
    ...
    uint16_t * const Data)
{
    ...
    Address = (uint32_t)(EEPROM_START_ADDRESS + (uint32_t)((1 + ValidPage) * PAGE_SIZE) - 2);
...
/**
  ...
  */
static uint16_t EE_VerifyPageFullWriteVariable(
    const uint16_t VirtAddress,
    const uint16_t Data)
{
    FLASH_Status FlashStatus;
    ...
    /* Get the valid Page end Address */
    PageEndAddress = (uint32_t)(EEPROM_START_ADDRESS + (uint32_t)((1 + ValidPage) * PAGE_SIZE) - 2);
...
static uint16_t EE_TransferVariable(
    const uint16_t VirtAddress,
...
/**
  ...
  *           - Flash error code: on write Flash error
  */
static uint16_t EE_PageTransfer(
    const uint16_t VirtAddress,
    const uint16_t Data)
{
    FLASH_Status FlashStatus;
